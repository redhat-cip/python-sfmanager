        ...
        with patch('sfmanager.sfmanager.get_cookie') as c:
            ...
            with patch('requests.' + method_verb) as method:
                ...
                self.assertTrue(action_func(parsed, self.base_url,
                                            self.headers))
                for k in method.call_args[1]:
                    args = method.call_args[1][k]
                    if k == 'data':
                        # data is serialized when calling method
                        args = json.loads(args)
                    # Handle unserialization random ordering for specific
                    # test case
                    if (isinstance(args, dict) and len(args.keys()) == 1 and
                            'members' in args):
                        self.assertSetEqual(set(args['members']),
                                            set(params[k]['members']))
                    else:
                        self.assertEqual(params[k], args)
                self.assertEqual(expected_url, method.call_args[0][0])
